...
class MedFormer_Decoder(nn.Module):
    ...
    def forward(self, x0, x1, x2, x3, x4, map_list):
...
class MedFormer_Encoder(nn.Module):        
    def __init__(self, 
        in_chan, 
        num_classes, 
        base_chan=32, 
    ...
    def forward(self, x):
        ...
        return x0, x1, x2, x3, x4, map_list
...
class MedFormer(nn.Module):
    ...
    def __init__(self, 
        ...
        # if conv_block == 'BasicBlock':
        #     dim_head = [chan_num[i]//num_heads[i] for i in range(8)]
        self.encoder = MedFormer_Encoder(
            in_chan,
            num_classes,
            base_chan,
            map_size,
        ...
        self.decoder =MedFormer_Decoder(
            in_chan,
            num_classes,
            base_chan,
            map_size,
    ...
    def forward(self, x):
        ...
        # x3, map3 = self.down3(x2)
        # x4, map4 = self.down4(x3)
        x0, x1, x2, x3, x4, map_list = self.encoder(x)
        ...
        # decoder part
        out = self.decoder(x0, x1, x2, x3, x4, map_list)
