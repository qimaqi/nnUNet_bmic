...
class Hiera(nn.Module, PyTorchModelHubMixin):
    ...
    def __init__(
        ...
        q_stride: Tuple[int, ...] = (2, 2), # TODO what is q_stride?
        ...
        patch_stride: Tuple[int, ...] = (4, 4), # patch stride control the 
        ...
        self.tokens_spatial_shape = [i // s for i, s in zip(input_size, patch_stride)] # [16, 224, 224] -> [8, 56, 56]
        num_tokens = math.prod(self.tokens_spatial_shape)
        flat_mu_size = math.prod(mask_unit_size) # what is flat mu size [1, 8, 8]
        ...
        self.mask_spatial_shape = [
            i // s for i, s in zip(self.tokens_spatial_shape, self.mask_unit_size) # [8, 56, 56] -> [8, 7, 7], each window have 64 token
        ]
        # print("self.mask_spatial_shape", self.mask_spatial_shape) # [8, 7, 7]
        # print("self.tokens_spatial_shape", self.tokens_spatial_shape) #  [8, 56, 56]
        # token spatial shape / 8
        ...
        # reoll, can reroll the masked token back to spatial order
        self.reroll = Reroll(
            input_size,
            patch_stride,
    ...
    def get_random_mask(self, x: torch.Tensor, mask_ratio: float) -> torch.Tensor:
        ...
        num_windows = math.prod(self.mask_spatial_shape)  # num_windows, each windows  [1, 7, 7]
    ...
    def forward(
        ...
        if mask is not None:
            ...
            # print("mask x.shape", x.shape) # [8, 4992, 144])
            # print("mask x.shape", x.shape) # ([8, 4992, 144])
