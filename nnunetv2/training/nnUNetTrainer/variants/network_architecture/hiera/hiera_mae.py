...
def apply_fusion_head(head: nn.Module, x: torch.Tensor) -> torch.Tensor:
    ...
    # permute [0, 3, 1, 2]
    # x.shape torch.Size([6, 8, 56, 56, 112])
    x = x.reshape(B * num_mask_units, *x.shape[2:]).permute(permute)
    x = head(x)
...
class MaskedAutoencoderHiera(Hiera):
    ...
    def get_pixel_label_3d(
        ...
        # print("input_vid", input_vid.shape)
        # ([8, 3, 8, 224, 224]) similar to video mae jump frame

        size = self.pred_stride
        # print("self.pred_stride", self.pred_stride)
        # print("input_vid.unfold(3, size, size)", input_vid.unfold(3, size, size).shape)
        label = input_vid.unfold(3, size, size).unfold(4, size, size)
        # unfold one dimension to patch, [8, 3, 8, 224, 224] -> [8, 3, 8, 14, 14, 16, 16]
    ...
    def forward_encoder(
        ...
        if mask is None:
            ...
            # print("mask", mask.shape) # torch.Size([8, 392]) # 
        ...
        # for i, inter_i in enumerate(intermediates):
        #     print( i, "intermediate feature", inter_i.shape)
        # 0 intermediate feature torch.Size([8, 78, 1, 8, 8, 144])
        # 1 intermediate feature torch.Size([8, 78, 1, 4, 4, 288])
        # 2 intermediate feature torch.Size([8, 78, 1, 2, 2, 1152])

        # Multi-scale fusion
        x = 0.0
        # video pretrain, q pool = 2, means only take 2 head  + last
    ...
    def forward_loss(
        ...
        x: e.g. [B, 3, H, W]
        pred: [B * num_pred_tokens, num_pixels_in_pred_patch * in_chans] # num_pixels_in_pred_patch
