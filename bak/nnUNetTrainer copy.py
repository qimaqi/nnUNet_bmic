...
from nnunetv2.utilities.get_network_from_plans import get_network_from_plans, get_network_linear_attention_from_plans, get_network_video_mae_from_plans, get_network_hiera_from_plans, get_network_mamba_from_plans
...
class nnUNetTrainer(object):
    def __init__(self, plans: dict, configuration: str, fold: int, dataset_json: dict, unpack_dataset: bool = True,
        ...
        if 'dynamic_network_architectures' in self.configuration_manager.network_arch_class_name:
            print("=====================================================================")
            print("Using nnUnet default parameters")
            ...
            self.enable_deep_supervision = True # For ViT 
            self.optimizer_type = 'SGD'
            self.lr_scheduler_type = 'Poly'

        elif 'Lin' in self.configuration_manager.network_arch_class_name:
            print("=====================================================================")
            print("Using Linear Attention default parameters")
            self.model_type = 'nnUNet_LinAttn'
        ...
        elif 'mae' in self.configuration_manager.network_arch_class_name:
            print("=====================================================================")
            print("Using Video MAE default parameters")
            self.model_type = 'ViT'
            self.initial_lr = 5e-5
            self.weight_decay = 3e-5
            self.oversample_foreground_percent = 0.33
            self.num_val_iterations_per_epoch = 50
            if 'e1500' in configuration:
                print("=====================================================================")
                print("Using Epoch 1500 and Iteration 500")
                self.num_epochs = 1500
                self.num_iterations_per_epoch = 500
            elif 'e1200' in configuration:
                self.num_epochs = 1005
                self.num_iterations_per_epoch = 350
            else:
                print("=====================================================================")
                print("Using default Epoch 1000 and Iteration 250")
                print(configuration)
                self.num_epochs = 1000
                self.num_iterations_per_epoch = 250
            self.current_epoch = 0
        ...
        elif 'hiera' in self.configuration_manager.network_arch_class_name:
            print("=====================================================================")
            ...
            self.lr_scheduler_type = 'Cosine'
            self.warmup_epochs = 10
        elif 'mamba' in self.configuration_manager.network_arch_class_name:
            print("=====================================================================")
            print("Using Mamba default parameters")
            ...
            raise ValueError(f"Unknown network architecture {self.configuration_manager.network_arch_class_name}")
    ...
    def initialize(self):
        if not self.was_initialized:
            ...
            # print("self.configuration_manager.network_arch_class_name", self.configuration_manager.network_arch_class_name) 
            # dynamic_network_architectures.architectures.unet.ResidualEncoderUNet
            # print("self.configuration_manager.network_arch_init_kwargs", self.configuration_manager.network_arch_init_kwargs)
            ...
            # print("self.configuration_manager.network_arch_init_kwargs_req_import", self.configuration_manager.network_arch_init_kwargs_req_import)
            # ['conv_op', 'norm_op', 'dropout_op', 'nonlin']
            # print("self.num_input_channels", self.num_input_channels)
            # self.num_input_channels 1
            # self.label_manager.num_segmentation_heads 16
            # self.enable_deep_supervision True
            # print("self.label_manager.num_segmentation_heads", self.label_manager.num_segmentation_heads)
            # print("self.enable_deep_supervision", self.enable_deep_supervision)
    ...
    def build_network_architecture(architecture_class_name: str,
        ...
        """
        if 'dynamic_network_architectures' in architecture_class_name:     
        ...
        elif 'Lin' in architecture_class_name:
            return get_network_linear_attention_from_plans(
                architecture_class_name,
                arch_init_kwargs,
                arch_init_kwargs_req_import,
                num_input_channels,
                ...
                allow_init=True,
                deep_supervision=enable_deep_supervision)
        elif 'mae' in architecture_class_name:
            return get_network_video_mae_from_plans(
                ...
                allow_init=True,
                deep_supervision=enable_deep_supervision)
        elif 'hiera' in architecture_class_name:
            return get_network_hiera_from_plans(
                ...
                allow_init=True,
                deep_supervision=enable_deep_supervision)
        elif 'mamba' in architecture_class_name:
            print("start init mamba")
            return get_network_mamba_from_plans(
                architecture_class_name,
                arch_init_kwargs,
                arch_init_kwargs_req_import,
                num_input_channels,
            ...
            raise ValueError(f"Unknown architecture class {architecture_class_name}")
    ...
    def _get_deep_supervision_scales(self):
        if self.enable_deep_supervision:
            ...
            elif self.model_type == 'nnUNet_LinAttn':
                deep_supervision_scales = list(list(i) for i in 1 / np.cumprod(np.vstack(
                    self.configuration_manager.pool_op_kernel_sizes), axis=0))[:-1]
                # print("deep_supervision_scales", deep_supervision_scales)
                # [[np.float64(1.0), np.float64(1.0), np.float64(1.0)], [np.float64(1.0), np.float64(0.5), np.float64(0.5)], [np.float64(0.5), np.float64(0.25), np.float64(0.25)], [np.float64(0.25), np.float64(0.125), np.float64(0.125)], [np.float64(0.125), np.float64(0.0625), np.float64(0.0625)]]
                # for 64x160x160 example: -> [8, 10, 10] why triplet, mean 3 dimension
            elif self.model_type == 'ViT' or self.model_type == 'Hiera':
                # print("self.configuration_manager", self.configuration_manager.network_arch_init_kwargs)
    ...
    def get_dataloaders(self):
        ...
        # initial_patch_size [159 257 200]
        # mirror_axes (0, 1, 2)
        # is_cascaded False
        # foreground_labels [np.int64(1), np.int64(2), np.int64(3), np.int64(4), np.int64(5), np.int64(6), np.int64(7), np.int64(8), np.int64(9), np.int64(10), np.int64(11), np.int64(12), np.int64(13), np.int64(14), np.int64(15)]
    ...
    def on_train_epoch_start(self):
        ...
        self.print_to_log_file(
            f"Current learning rate: {np.round(self.optimizer.param_groups[0]['lr'], decimals=5)} and {np.round(self.optimizer.param_groups[-1]['lr'], decimals=5)}")
    ...
    def train_step(self, batch: dict) -> dict:
        ...
        #     print(f"Data {i}: {data_i.shape}")
        # for i, target_i in enumerate(target):
        #     print(f"Target {i}: {target_i.shape}")

        # deep supervison
        # Data 0: torch.Size([1, 64, 160, 160])
        # Data 1: torch.Size([1, 64, 160, 160])
        # Target 0: torch.Size([2, 1, 64, 160, 160])
        # Target 1: torch.Size([2, 1, 64, 80, 80])
        # Target 2: torch.Size([2, 1, 32, 40, 40])
        # Target 3: torch.Size([2, 1, 16, 20, 20])
        # Target 4: torch.Size([2, 1, 8, 10, 10])

        # no deep supervision
        # Data 0: torch.Size([1, 64, 160, 160])
        # Data 1: torch.Size([1, 64, 160, 160])
        # Target 0: torch.Size([1, 64, 160, 160])
        # Target 1: torch.Size([1, 64, 160, 160])
